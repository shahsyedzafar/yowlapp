...
//	Set this to your actual CONSUMER KEY and SECRET for your application as given to you by dev.twitter.com
	private static final String CONSUMER_KEY		= "G9VclHuMHK7frvI5Xpgtn8XgK";
	private static final String CONSUMER_SECRET 	= "9A1nXbNlTD9H0QklQN1UFvyYKPlpp403pLw6NOIx1XOxOPMHVj";
	...
	public void getData(String searchterm, GenerateCSVFile gcf) {
...
//				If maxID is -1, then this is our first call and we do not want to tell Twitter what the maximum
				...
				{
					// For each tweet we create a hashmap
					...
					*/
					...
					//has_timestamp
					csvReport.append(s.getCreatedAt().toString()+ ",");
					hm.put("Timestamp", cleanText(s.getCreatedAt().toString()));
					
					csvReport.append(searchterm.toString()+",");
					hm.put("TouristSpotName", cleanText(searchterm.toString()));
					
					
					//has_hashtag
					HashtagEntity[] ht_arr = s.getHashtagEntities();
					//System.out.println("Printing hashtag arraylength");
					//System.out.println(ht_arr.length);
					
				
					if(ht_arr.length>0) {
						//System.out.println("printing one hashtag");
						//System.out.println(ht_arr[0].getText().toString());
						
						//csvReport.append(cleanText(ht_arr[0].getText().toString()) + ",");
						hm.put("HashTag",cleanText(ht_arr[0].getText().toString()) );
						
						
					} else {
						//csvReport.append("Absent!" + ",");
						
						hm.put("HashTag","Absent!");
					}
					
					//System.out.println("Printing hashtag text");
					//has_text
					//csvReport.append(cleanText(s.getText()) + ",");
					
				
						System.out.println(s.getText());
						hm.put("Text", cleanText(s.getText()));
					...
					if(me.length>0) {
						for(int i=0;i<me.length;i++) {
							if(me[i].getMediaURL().toString()!=null) {
								
								//System.out.println("Printing one media text");
								//System.out.println(me[i].getURL().toString());
								hm.put("MediaUrl",me[i].getURL().toString());
								
								csvReport.append(cleanText(me[i].getURL().toString()) + ",");
								break;
								
							}
						}		
					} else {
						hm.put("MediaUrl", "Absent");
						csvReport.append("Absent!" + ",");
					}
					...
					if(s.getGeoLocation()!=null) {
						csvReport.append(cleanText(s.getGeoLocation().toString()) + "," + "\n");
						hm.put("GeoLocation", cleanText(s.getGeoLocation().toString()));
						
					}
					else {
						
						hm.put("GeoLocation", "Absent!");
						csvReport.append("Absent!" + "," + "\n");
					...
					sb.append(hm.get("TouristSpotName")+",");
					sb.append(hm.get("HashTag")+",");
					sb.append(hm.get("Text")+",");
					sb.append(hm.get("MediaUrl")+",");
					sb.append(hm.get("GeoLocation")+"\n");

					System.out.println("Printing complete stringBuffer------------------");
